    ...
    if Config.MultiItemSetList[type] then
        ...
        if not foundResult then
            ...
            for obj_key, obj_table in pairs (Config.MultiItemSetList[type]) do
               ...
               prop = CreateObject(obj_table.ObjectCode, coords.x, coords.y, coords.z , 0.2, true, true, false, false, true)
...
-- Testing purposes
if Config.UsewCommand then

    RegisterCommand("w", function(source, args)    

        if args ~= nil and args[1] ~= nil then
                armed = true
            AttachHeadbandTypeOnPlayerPed("w" .. tostring(args[1]))
        end
    end)
